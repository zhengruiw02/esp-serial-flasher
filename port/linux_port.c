...
static void gpio_set_direction(int pin, const char *direction) {
    char path[64];
    printf("Set direction pin %d as %s\n", pin, direction);
...
static void gpio_write_value(int pin, int value) {
    char path[64];
    printf("Write pin %d as %d\n", pin, value);
...
esp_loader_error_t loader_port_linux_init(const loader_linux_config_t *config)
{
    ...
    // reset pin
    gpio_export(s_reset_trigger_pin);
    gpio_set_direction(s_reset_trigger_pin, "out");
    // gpio0 pin
    gpio_export(s_gpio0_trigger_pin);
    gpio_set_direction(s_gpio0_trigger_pin, "out");
...
{
    gpio_write_value(s_gpio0_trigger_pin, SERIAL_FLASHER_BOOT_INVERT ? 1 : 0);
    loader_port_reset_target();
    loader_port_delay_ms(SERIAL_FLASHER_BOOT_HOLD_TIME_MS);
    gpio_write_value(s_gpio0_trigger_pin, SERIAL_FLASHER_BOOT_INVERT ? 0 : 1);
}


void loader_port_reset_target(void)
{
    gpio_write_value(s_reset_trigger_pin, SERIAL_FLASHER_RESET_INVERT ? 1 : 0);
    loader_port_delay_ms(SERIAL_FLASHER_RESET_HOLD_TIME_MS);
    gpio_write_value(s_reset_trigger_pin, SERIAL_FLASHER_RESET_INVERT ? 0 : 1);
}
